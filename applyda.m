...
fulldata=data{1,2};
ratings=fulldata(:,2); %all ratings
...
%calling dragon fly algorithm
Best_pos=DAalgo(datas20,tr20,tnr20,nar);
neighbours={};
k=1;
...
% Restaurent Recommendation generation
Vy=1.1;
highly=[];
recomm=[];
recomm_not=[];
...
for i=1:size(prerates,1)
    for j=1:size(prerates,2)
        ...
        elseif (prerates(i,j)-Vy)<-1 && (prerates(i,j)-stddev(j))<=0
            if isempty(find(highly==pl(j),1)) && isempty(find(recomm==pl(j),1)) && isempty(find(recomm_not==pl(j),1))
...
for i=1:size(prerates,1)
    for j=1:size(prerates,2)
        if ~isnan(prerates(i,j))
            ...
            if prerates(i,j)~=0 && count<length(bestneighbours)*10
                count=count+1;
            end
         end
    end
